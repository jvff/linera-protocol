...
use {
    crate::{maybe_flat_type::MaybeFlatType, ExportFunction, RuntimeError},
    wasmtime::{Caller, Linker, WasmRet, WasmTy},
};
...
macro_rules! export_function {
    ...
    (@generate $( $names:ident: $types:ident ),*) => {
        ...
        where
            ...
            Handler:
                Fn(Caller<'_, Data>, ($( $types, )*)) -> Result<FlatResult, anyhow::Error>
                + Send
                + Sync
                + 'static,
        {
