use {
    super::{Getters, Operations, Setters, SimpleFunction},
    witty::wasmtime::InstanceWithStore,
};

#[test]
fn simple_function() {
    let store = load_test_module("simple-function");

    SimpleFunction::new(store)
        .simple()
        .expect("Failed to run guest's `simple` function");
}
...
fn load_test_module(name: &str) -> InstanceWithStore<()> {
    let engine = wasmtime::Engine::default();
    let mut store = wasmtime::Store::new(&engine, ());
    let module = wasmtime::Module::from_file(
        &engine,
        format!("test-modules/target/wasm32-unknown-unknown/debug/export-{name}.wasm"),
    )
