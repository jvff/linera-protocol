#[path = "reentrancy/wasmer.rs"]
mod wasmer;
#[path = "reentrancy/wasmtime.rs"]
mod wasmtime;

use {
    witty::{Caller, HList, RuntimeError, RuntimeInstanceWithFunctionScope, WitExport},
...
impl ExportedSimpleFunction {
    fn simple(
        caller: impl Caller<()> + RuntimeInstanceWithFunctionScope<HList![], HList![]>,
    ) -> Result<(), RuntimeError> {
